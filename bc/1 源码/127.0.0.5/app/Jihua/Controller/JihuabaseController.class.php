...
class JihuabaseController extends Controller {
	...
	function checkjihua($totalzxnum=0){
		...
		$get_lhc_time= self::get_lhc_time();
		echo self::GBK($get_lhc_time);
		//日亏损赠送活动
		$_setdatetime    = 0;
		$_set_start_time = 0;
		$_set_end_time   = 0;
	...
	protected function get_lhc_time(){
		$url="http://1680660.com/smallSix/findSmallSixInfo.do";//获取当月开奖时间
        $co= file_get_contents($url);
        $RES = json_decode($co,true);
		if(!$RES)return '六合彩时间获取失败';
		//var_dump($co);exit;
		...
		$query=M("lhctime")->where(['qihao'=>$currFullExpect])->find();
		if($query)return '六合彩时间已存在';
		$data=['preDrawIssue'=>$lastFullExpect,'qihao'=>$currFullExpect,'drawTime'=>strtotime($openRemainTime)];
		if(M("lhctime")->add($data))return '六合彩时间获取成功';
