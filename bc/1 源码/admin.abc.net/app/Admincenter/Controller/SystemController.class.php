...
class SystemController extends CommonController {
	...
	function setting(){
		...
		$newarray['agent_commission_type']=json_decode($newarray['agent_commission_type'],true);
		//dump($newarray['agent_commission_type']);
	...
	function settingdo(){
		...
		if(empty($param['bancountry'])){
		    $param['bancountry'] = array();
        }
		if($param['agent_commission_type']){
		    $param['agent_commission_type'] = json_encode($param['agent_commission_type']);
			//var_dump($param['agent_commission_type']);die;
        }
	...
	function lhctime(){
		...
		$show       = $Page->show();
		$list       = $db->where($map)->limit($Page->firstRow.','.$Page->listRows)->order("qihao desc")->select();
	...
	function lhctime_add(){
		if(IS_POST){
			$post=I('post.');
			if(!$post['qihao'] or !$post['drawTime'] ){
				$this->error('数据填写不完整');exit;
			}
			$data=$post;
			$data['preDrawIssue']=$post['qihao']-1;
			$data['drawTime']=strtotime($post['drawTime']);
			if(M('lhctime')->data($data)->add()){
...
function lhctime_del(){
		$id=I('id');
			if($id){
				$res=M('lhctime')->where(['id'=>$id])->find();
				if(!$res)$this->error('数据不存在或已经删除 ');
				if(M('lhctime')->where(['id'=>$id])->delete()){
					$this->success('删除成功！');
				}else{
					$this->error('删除失败');
				}
