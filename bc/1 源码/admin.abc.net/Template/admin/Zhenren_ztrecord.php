...
<div class="page-container">
	<form method="get" action="{:U('ztrecord')}" class="text-c">
...
<input class="input-text" type="text" style="width:180px;" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="{$_eDate}" name="eDate">&nbsp;&nbsp;
		...
		平台<input class="input-text" type="text"  value="{$type}" name="type" style="width:100px">
        
        <a class="btn btn-default-outline radius" href="{:U('ztrecord')}">重置</a>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
            ...
            <tr class="text-c">
				...
				<td>{$vo.winAmount}</td>
                <td>{$vo.netPnl}</td>
				  <td>{$vo.betTime|date='Y-m-d H:i:s',###}</td>
