...
<a href="javascript:;" layer-url="{:U('useradd',['parentid'=>$parentid])}" title="添加会员" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加会员</a>&nbsp;&nbsp;&nbsp;
{else /}
		...
		<tbody>
            ...
            <tr class="text-c">
               ...
               <!-- <td>{$grouplist[$vo['groupid']]['groupname']}</td>-->
                ...
                <br>
                <u style="cursor:pointer" class="text-primary" layer-del-url="{:U('userdelete',['id'=>$vo['id']])}" title="删-{$vo.username}">删</u> | <u style="cursor:pointer" class="text-primary" layer-alt-url="{:U('unline',['id'=>$vo['id']])}" title="踢-{$vo.username}">踢</u> | <u style="cursor:pointer" class="text-primary {if condition='$vo[islock] eq 0'}c-999{elseif condition='$vo[islock] eq 1' /}c-green{/if}" lock-url="{:U('lock',['id'=>$vo['id']])}" title="锁定/解锁-{$vo.username}">{if condition="$vo['islock'] eq 0"}锁定{elseif condition="$vo['islock'] eq 1" /}解锁{/if}</u>| <u style="cursor:pointer" class="text-primary" layer-url="{:U('Message/add',['RecID'=>$vo['id']])}" layer-width="100%" layer-height="100%" title="发送消息">发信</u> |
                <u style="cursor:pointer" class="text-primary" lock-url="{:U('freezePresent',['id'=>$vo['id']])}" title="冻结提现/解除冻结提现-{$vo.username}">{if condition="$vo['is_freeze_present'] eq 0"}冻结提现{elseif condition="$vo['is_freeze_present'] eq 1" /}解除冻结提现{/if}</u>
                </td>
			</tr>
            {/volist}
		</tbody>
	...
	layer.confirm(title,function(index){
		$.getJSON(url, function(json){
			if(json.status==1){
			    ...
			    if(obj.text()=='冻结提现' || obj.text() == '解除冻结提现'){
                    if(obj.text()=='冻结提现'){
                        obj.text('解除冻结提现');
                    }else{
                        obj.text('冻结提现');
