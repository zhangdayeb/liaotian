...
//		echo jsonreturn($Result);
	...
	function savetikuanorder(){
		...
		if($userinfo['is_freeze_present'] == 1){
		    echo jsonreturn(['sign'=>false,'message'=>'提现冻结中，请联系客服']);exit;
        }
	...
	function gettouzhulog(){
		$userinfo=session('userinfo');
		$name=I('name');
		$res=M('touzhu')->where(['cpname'=>$name,'uid'=>$userinfo['id']])->limit(10)->order('id desc')->select();
		
		 echo json_encode(["data"=>$res,'sign'=>true]);
	}
	function getUserMoeny(){
		$userinfo=session('userinfo');
		$name=I('name');
		$res=M('member')->where(['id'=>$userinfo['id']])->find();
		if(!$res)exit(json_encode(['sign'=>false,'message'=>'数据不存在']));
		 echo json_encode(["data"=>$res,'sign'=>true]);
