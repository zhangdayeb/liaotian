...
class MemberController extends CommonController
{
  ...
  function memberList($apiparam = array())
  {
    ...
    foreach ($Result as $k => $v) {
        ...
        if (!empty($v['parentid'])) {
          $parent = $db->where("parentid='{$v['id']}'")->select();
          $parentname2 = $db->field('username')->where("parentid='{$v['id']}'")->select();
          $v['parentname'] = $parentname2['username'];
        }
        $Result2[] = $v;
...
// die;
  ...
  function getChilduids($parentid, $allids)
  {
    $map = [];
    $map['parentid'] = ['eq', $parentid];
    $map['isnb'] = ['eq', 0];
    $chiduids = M('member')->where($map)->getField('id', true);
    foreach ($chiduids as $chiduid) {
      $allids[] = $chiduid;
      $allids = self::getChilduids($chiduid, $allids);
    }
    return $allids;
  }

  function groupreport($apiparam = array())
  {
    $apiparam = self::_cheacktoken($apiparam);
    if (!$apiparam['sign']) return $apiparam;
    $apiparam = checklogin($apiparam);
...

    {
      ...
      $map['id'] = ['in', $chiduids];
      if ($loginname) $map['username'] = ['eq', $loginname];
      $map['regtime'][] = ['elt', $endtime];
      $map['regtime'][] = ['egt', $startime];
      $regNum = M('member')->where($map)->count();
      //下级人数
      $map = [];
      $map['id'] = ['in', $chiduids];
      if ($loginname) $map['username'] = ['eq', $loginname];
      $subNum = M('member')->where($map)->count();
      //$list['subNum'] = sizeof($chiduids) - 1;
      ...
      //$map['regtime'][] = ['elt', $endtime];
      //$map['regtime'][] = ['egt', $startime];
      $groupBalance = M('member')->where($map)->sum('balance');

      $list['dayRechargeMoney'] = $dayRechargeMoney ? $dayRechargeMoney : 0;
      $list['dayDrawRechargeMoney'] = $dayDrawRechargeMoney ? $dayDrawRechargeMoney : 0;
      $list['dayConsumptionMoney'] = $dayConsumptionMoney ? $dayConsumptionMoney : 0;
      $list['dayCommissionMoney'] = $dayCommissionMoney ? $dayCommissionMoney : 0;
      $list['dayIncomeMoney'] = $dayIncomeMoney ? $dayIncomeMoney : 0;
      $list['dayActivitiesMoney'] = $dayActivitiesMoney ? $dayActivitiesMoney : 0;
      $list['dayDividendMoney'] = $dayDividendMoney ? $dayDividendMoney : 0;
...
//      return $apiparam;
  ...
  {
    $c_prefix = 'cp58_';
    $uid = cookie($c_prefix . 'uid');
    $tid = cookie($c_prefix . 'tid');
    $sessionint = M('membersession')->where(['userid' => $uid])->find();
    if (!empty($sessionint)) {
      $sid = $sessionint['sessionid'];
      if ($tid == md5($sid)) {
        return true;
      }
    }
