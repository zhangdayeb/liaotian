...
<body class="bg_fff">
	...
	<div data-am-widget="" class="am-tabs am-tabs-d2 billrecord_main">
		<ul class="am-tabs-nav am-cf am-avg-sm-3" style="text-align:center;">
			...
			<li class="am-active"><a href="<?php echo U('Mobile/Account/dealRecord3');?>">提现记录</a></li>
		</ul>
					...
					<div class="am-list-news-bd">
						<ul class="am-list">
							<?php if(is_array($mxlist)): $i = 0; $__LIST__ = $mxlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="am-g am-list-item-dated">
								<p class="am-cf">
									<span class="what_type am-fl"><?php echo ($vo['bankname']); ?></span>
								...
								<p class="am-cf billrecord_time">
									...
									<?php if($vo['state'] == 1): ?><em class="am-fr" style="color:green">提款成功</em>
										<?php elseif($vo['state'] == 0): ?>
										<em class="am-fr" style="state:green">处理中</em>
										<?php elseif($vo['state'] == -1): ?>
										<em class="am-fr" style="color:grey">提款退回</em><?php endif; ?>
								</p>
								<?php if($vo['remark'] != ''): ?><p class="am-cf">
                                    <span style="font-size: 0.12rem">备注：<?php echo ($vo['remark']); ?></span>
                                </p><?php endif; ?>
...
function Order_chedan(id,trano,obj){
	...
	<script>
		...
		function chaxun(t){ 
			var atime = t;
			var url = "/?m=Mobile&c=Account&a=dealRecord3&atime="+atime ;
