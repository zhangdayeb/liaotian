...
class LotteryController extends CommonController {
	...
	function cpbuy($apiparam=array()){
		...
		//午夜约吧判断
		$lotteryname = $apiparam["orderList"]["lotteryname"];        //午夜约吧标识
		$expect = $apiparam["orderList"]["expect"];                  //期号
		$cpinfo = M('caipiao')->where(['name'=>$lotteryname])->find();  //获取午夜约吧信息
		if(!$cpinfo){
			return(['sign'=>false,'message'=>'午夜约吧不存在']);
		}
		if($cpinfo['isopen']==0){
			return(['sign'=>false,'message'=>'当前午夜约吧已关闭投注']);
		}
		if($cpinfo['iswh']==1){
			return(['sign'=>false,'message'=>'当前午夜约吧维护中']);
		}
		//时间判断
		$_lotterytimesclass = "Lib\\lotterytimes\\{$lotteryname}";
		$_lotterytimes = new $_lotterytimesclass;                //获取当前午夜约吧类文件
		$_lottetimes = $_lotterytimes->drawtimes();             //获取午夜约吧剩余时间
       //echo ($_lottetimes['currFullExpect'].'----'.$expect);exit;
		if($_lottetimes['currFullExpect']!=$expect){            //根椐期号判断午夜约吧是否过期
			$apiparam['sign'] = false;
			$apiparam['message'] = '当前午夜约吧已截至投注1';
			return $apiparam;
		}
		$lastkjinfo = M('kaijiang')->where(['name'=>$lotteryname,'expect'=>$expect])->find();

		if($lastkjinfo){ //判断当前午夜约吧期号是存在
			$apiparam['sign'] = false;
			$apiparam['message'] = '当前午夜约吧已截至投注2';
			return $apiparam;
		}
		//取玩法
		...
		$membergroup = M('membergroup')->where(['groupid'=>$userinfo['groupid']])->find();//获取会员组信息
		$_rateconfigs = unserialize($membergroup['configs']);  //会员组设置
        $open = C('agent_commission_open');
		foreach($rates as $k0=>$v0){
			$rateinfo = [];
			$rateinfo = M('wanfa')->where(['typeid'=>$typeid,'playid'=>$v0['playid']])->find();
            ...
            if($typeid == 'lhc' || $typeid == 'k3'){
                if($open){
					if(strstr($v0['rate'],'|')){
								$rate_arr = explode('|',$v0['rate']);
								foreach($rate_arr as $rate_k =>$rate_v){
									$rate_arr['data'][]= sprintf("%.2f",$rate_v-$rate_v*(GetVar('fanDianMax')-$userinfo['fandian'])/100);
								}
								 $rateinfo['rate'] = implode('|',$rate_arr['data']);
			...
			if(intval($rates[$v['playid']]['maxxf'])>0){
				...
				$_oktztotal = M('touzhu')->where($_grouptzmap)->sum('amount');
					  ...
					  if(strstr($_REQUEST['lotteryname'],'xyft'))$zyclass ="pk10";
					    if(strstr($_REQUEST['lotteryname'],'jsft'))$zyclass ="pk10";
...
/*				   if($v['rate']!=$rates[$v['playid']]['rate']){   //判断实际赔率是否正确
				...
				if($v['fandian']){
				    $rate = $rate - sprintf("%.2f",round($rate) / 100 * $v['fandian']);
				    $a = strval($rate);
				    $b = strval($v['rate']);
				    //$a = intval($rate * 100);
				    //$b = intval($v['rate'] * 100);
                    if($a != $b){
                        $this->ajaxReturn(['sign'=>false,'message'=>"赔率非法1"]);exit;
                    }
                }elseif($v['rate'] != $rate ){
		...
		foreach($_REQUEST['orderList'] as $k=>$v){
			...
			$type_fanyong=json_decode(GetVar('agent_commission_type'),true);
			if(in_array($typeid,$type_fanyong) && $userinfo['parentid'] !='0' && $agentCommissionOpen){
			...
			$this->dailifandian($userinfo['parentid'],$userinfo['fandian'],$data['amount'],$trano,$userinfo['id'],$userinfo['username'],$userinfo['fandian'],$i);
             foreach($this->_parent as $k => $v){
				...
				$fuddetaildata['remark']       = $remark?$remark:$userinfo['username'].'下注-佣金发放通过';
			...
			if($_int){//操作账户金额、日志等
				...
				$fuddetail_data['remark'] = "下单扣费，午夜约吧:{$cpinfo['title']},{$expect}";
            ...
            //洗码
                if($_membercenter['xima']>0){
                    ...
                    $fuddetail_data['oddtime'] = $_t;
                    $fuddetail_data['remark'] = "下单减，午夜约吧:{$cpinfo['title']},{$expect}";
				...
				/*//每消费增加积分
					...
					if($pointtouzhu>0 && $pointtouzhuadd>0){
						...
						if($_addpoint>0){
							...
							$fuddetail_data['oddtime'] = $_t;
							$fuddetail_data['remark'] = "投注送积分，午夜约吧:{$cpinfo['title']},{$expect}";
    ...
    function oldcpbuy($apiparam=array()){
        ...
        $lotteryname = $apiparam["orderList"]["lotteryname"];        //午夜约吧标识
        $expect = $apiparam["orderList"]["expect"];                  //期号
        $cpinfo = M('caipiao')->where(['name'=>$lotteryname,'is_old'=>1])->find();  //获取午夜约吧信息
        if(!$cpinfo){
            return(['sign'=>false,'message'=>'午夜约吧不存在']);
        }
        if($cpinfo['isopen']==0){
