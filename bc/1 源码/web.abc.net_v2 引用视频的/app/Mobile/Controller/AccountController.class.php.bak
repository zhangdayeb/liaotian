...
//					echo '<script type="text/javascript">alert("银行卡真实姓名绑定成功！");window.location.href="member.index"</script>';
	...
	function dealRecord()
	{
		...
		foreach ($mxlist as $key => $value) {
			if($value['type']=='order' or $value['type']=='xima' or $value['type']=='withdraw' or $value['type']=='cancel')
			{
...
/*		foreach($_paylist as $k=>$v)
    ...
    function todayLoss()
	{
		...
		$chuzhicount     = M('recharge')->where($map)->where('state=1')->sum('amount'); //充值总金额
    ...
    $fanshui     = M('fanshui')->where($map)->where('shenhe=1')->sum('amount'); //每日加奖-反水总金额
   ...
   //快捷支付
	...
	function hxpay()
	{
		$Allmsg = M('payset')->field("paytype ,paytypetitle,configs ,minmoney,maxmoney,remark")->where("isonline=-1 AND state=1 AND paytype='hxpay'")->select();
		foreach ($Allmsg as $key => $value)
		{
			$Allmsg['paytype']  =  $value['paytype'];
			$Allmsg['paytypetitle']  =  $value['paytypetitle'];
