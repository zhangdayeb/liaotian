...
class MemberController extends CommonController {
	...
	function agent()
	{
	    ...
	    $no1 = M('member')->where(["parentid"=>$uid])->select();
	    foreach ($no1 as $k=>$v)
	    {
	        $no2 = M('member')->where("parentid='{$v['id']}'")->select();
	        if(!empty($no2)){
	           // $parentname2 = M('member')->field('username')->where("parentid='{$v['id']}'")->find();
            //     $no2[$k]['parentname'] = $parentname2['username'];
	           // $aa[$k] = $no2;
	        }else{
	           // $no2 = '无';
	        }
        ...
        $newArray = array();//三维转二维
        foreach($array as $a=>$k){
            $newArray[] = $k;
        }

        $result = array_reduce($newArray, 'array_merge', array());//二维转一维


	   // var_dump($result);
	   // die;
	    $this->assign('no2',$result);
	    $this->assign('no1',$no1);
	    $this->display();
	}
...
//			redirect(U("Member/addbank"));exit;
	...
	function Message()
    {
       ...
       foreach ($res as $k => $v) {
        	 ...
        	 if(!strpos($v['read'],'"'.$RecID.'"') /*and $v['RecID']!='*'*/){
        	 	$res[$k]['isyd']=0;
        	 }else{
        	 	$res[$k]['isyd']=1;
        	 }
    ...
    function Message_show(){
        ...
        $res=M('message a')
            ...
            ->find();
        if(!$res)
            return $this->error('未查询到消息数据');


         // 查询是否已读 如果未读标记为已读 写入会员id
        if(!strpos($res['read'],'"'.$RecID.'"') /*and $res['RecID']!='*'*/){

        	$data=json_decode($res['read'],1);
        	if(!$data){//空创建并插入
        		$data=[];
        		$data[]=$RecID;
        	}else{
        	...
        	//var_dump($data);
        	M('message')->where(['id'=>$res['id']])->save(['read'=>json_encode($data)]);
