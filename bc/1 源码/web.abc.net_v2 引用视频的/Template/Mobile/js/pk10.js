		...
		if(data.sign==true){
			...
			if (lottery.remainTime && eval(lottery.remainTime) > 1) {
				...
				if(yIndex>1){
					art.dialog({
						time: 2,
						content:lottery.lastFullExpect+'期下单时间已截止',
						lock:true
					});
				}
				yIndex++;
	...
	$.post(url, {'lotteryname':lotteryname}, function(data) {
		...
		if(data.sign==true){
			...
			for(var o in lotlist){
				...
				//console.log(yopentimes_arr[0]);
				var md_arr=yopentimes_arr[0].split('-')
				var md=md_arr[1]+'-'+md_arr[2]
				var yopentime1 = yopentimes_arr[1].split(":");
				var yopentime=  md+' '+yopentime1[0]+":"+yopentime1[1]
				//颜色
				code_arr=openinfo.opencode.split(',')
				var haoma_html='';
				for (var iii=0; iii<code_arr.length; iii++) { 
					haoma_html =haoma_html+"<span class='pkcode pkcode_"+code_arr[iii]+"'>"+code_arr[iii]+"</span>&nbsp;";
				}
				
				html += '<tr class="code_border"><td width="130" style="border-right: 1px solid #a7a182;">'+openinfo.expect+'<br>'+yopentime+'</td><td width="46" class="c_red fb" style="padding:5px;">'+haoma_html+'</td></tr>';
                ...
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
						...
						var yopentime1 = yopentimes_arr[1].split(":");
						 var yopentime=  md+' '+yopentime1[0]+":"+yopentime1[1]
						 //颜色
						code_arr=msg.data.opencode.split(',')
						var haoma_html='';
						for (var iii=0; iii<code_arr.length; iii++) { 
							haoma_html =haoma_html+"<span class='pkcode pkcode_"+code_arr[iii]+"'>"+code_arr[iii]+"</span>&nbsp;";
						}
				
						var html = '<tr class="code_border"><td width="130" style="border-right: 1px solid #a7a182;">'+openexpect+'<br>'+yopentime+'</td><td width="46" class="c_red fb" style="padding:5px;">'+haoma_html+'</td></tr>';
...
function stopLottery(codes) {
		$('#ssc_winning_sum li').removeClass()
	var nums = codes.split(',');
	if (nums.length >= 10) {
		$('#ssc_winning_sum li').eq(0).addClass('pk10_code_'+nums[0] );
		setTimeout(function() {
			clearInterval(T10);
...
//			}
	...
	if (nums.length >= 9) {
			$('#ssc_winning_sum li').eq(1).addClass('pk10_code_'+nums[1] );
		setTimeout(function() {
			clearInterval(T9);
...
//			}
	...
	if (nums.length >= 8) {
			$('#ssc_winning_sum li').eq(2).addClass('pk10_code_'+nums[2] );
		setTimeout(function() {
			clearInterval(T8);
...
//			}
	...
	if (nums.length >= 7) {
			$('#ssc_winning_sum li').eq(3).addClass('pk10_code_'+nums[3] );
		setTimeout(function() {
			clearInterval(T7);
...
//			}
	...
	if (nums.length >= 6) {
			$('#ssc_winning_sum li').eq(4).addClass('pk10_code_'+nums[4] );
		setTimeout(function() {
			clearInterval(T6);
...
//			}
	...
	if (nums.length >= 5) {
			$('#ssc_winning_sum li').eq(5).addClass('pk10_code_'+nums[5] );
		setTimeout(function() {
			clearInterval(T5);
	...
	if (nums.length >= 4) {
			$('#ssc_winning_sum li').eq(6).addClass('pk10_code_'+nums[6] );
		setTimeout(function() {
			clearInterval(T4);
...
//			}
	...
	if (nums.length >= 3) {
			$('#ssc_winning_sum li').eq(7).addClass('pk10_code_'+nums[7] );
		setTimeout(function() {
...
//			}
	...
	if (nums.length >= 2) {
			$('#ssc_winning_sum li').eq(8).addClass('pk10_code_'+nums[8] );
		setTimeout(function() {
			var openLotteryList = $('#ssc_winning_sum').find('li');
			clearInterval(T2);
...
//			}
	...
	if (nums.length >= 1) {
			$('#ssc_winning_sum li').eq(9).addClass('pk10_code_'+nums[9] );
		setTimeout(function() {
			clearInterval(T1);
