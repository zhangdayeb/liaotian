...
$(function(){
	...
	$(".play_select_tit li").click(function(){
		...
		_thisPlayid=lottery_code;
		$("#gn_main_cont").html(co);
		if(_thisPlayid!='k3hzzx'){
			silber(_thisPlayid);
		}else{
			silbers('k3hzbig');
		}
...
function submit_order(_playid,_number,number,zhushu,obj,ishz){
	if(!ishz){
		...
		var array = { 
			...
			'minjj':rate.minjj,
			'maxzs':rate.maxzs,
			'rate':lotteryRate==0?rate.rate:lotteryRate,
			'fandian':lotteryFandian
		}; 
		orderList=new Array();
...
$(document).on('click','.betting_right_btn',function () {
	...
	html += '<div class="clearfix"><div class="pull-left" style="width:100px;">投注内容:</div><div style="margin-left:100px; text-align: left;white-space:normal;word-break:break-all;color:red" id="ordertotalprice">'+lottery_number+'</div></div>';
...
var oDiv;
var fands=yrates['k3hzbig'].fandian*10;//parseFloat(yrates['tmzx'].fandian)/100;
var dufrate=80;
var fandn=Math.round(dufrate)/100;

function silber(name){
	    fands=yrates[name].fandian*10;//parseFloat(yrates['tmzx'].fandian)/100;
	    dufrate=parseFloat(yrates[name].rate);
...
function silbers(name){
    ...
    function setslide(data,dufrate,fandn,name){
      ...
      $('.tpl').text((dufrate-fandl).toFixed(2));
      lotteryFandian=(fans).toFixed(1);
      lotteryRate=(dufrate-fandl).toFixed(2);
    }
    function setslidedd(data){
    $('.ball_list_ul').find('li').each(function(i){
      var playid=$(this).children('a').attr('playid');
      var dufrate=parseFloat(yrates[playid].rate);
      var fandn=Math.round(dufrate)/100;
      var fans=data/10;
      var fandl=(fans*fandn).toFixed(2);
      $('.fans').text((fans).toFixed(1));
        //$( "#amount" ).text((dufrate-fandl).toFixed(2));
        $(this).find('.peilv').text('赔率'+(dufrate-fandl).toFixed(2));
        lotteryRate=404;
        lotteryFandian=(fans).toFixed(1);
    })
  }
