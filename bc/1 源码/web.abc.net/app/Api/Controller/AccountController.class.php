...
class AccountController extends CommonController {
	...
	'getrechargetypelist','addrecharge','notify','sendtrano','isUserWithdrawLimit','savetikuanorder',
	...
	function chedan($apiparam=array()){
		...
		if($_lottetimes['currFullExpect']!=$info['expect']){
			...
			$_t = time();
			M('touzhu')->where(['trano'=>$trano])->setField('isdraw',-2);
			...
			if($_int){
				...
				$fandian_list=M('dailifandian')->where(['trano'=>$trano])->select();
				if($fandian_list){
					foreach($fandian_list as $v){
						$dailiuserinfo=M('member')->where("id='{$v['uid']}'")->find();
						...
						$fuddetail_data['amountbefor'] = $dailiuserinfo['balance'];
						$fuddetail_data['amountafter'] = $dailiuserinfo['balance']-abs($v['amount']);
						$fuddetail_data['oddtime'] = $_t;
						$fuddetail_data['remark'] = $userinfo['username']."-撤单，扣除佣金";
						$fuddetail_data['type'] = 'cancel';
						$fuddetail_data['typename'] = C('fuddetailtypes.cancel');
						M('fuddetail')->data($fuddetail_data)->add();
						//删除佣金数据
						M('dailifandian')->where(['id'=>$v['id']])->delete();
					}
				}
				$apiparam['sign']=true;
...
/*			if($userinfo['question'] && is_array($userinfo['question'])){
	...
	function notify()
	{
		...
		{
		$id=I('id');
	$rec = M('recharge')->where(['id'=>$id,'state'=>0])->find();
	
	if (!empty($rec)){
		$user=M('member')->where(['id'=>$rec['uid']])->find();
		$balance=$user['balance']+$rec['amount'];
		M('member')->where(['id'=>$rec['uid']])->data(['balance'=>$balance])->save();
	M('recharge')->where(['id'=>$id])->data(['state'=>1,"oldaccountmoney"=>$user['balance'],"newaccountmoney"=>$balance,"remark"=>"在线充值增加"])->save();	
