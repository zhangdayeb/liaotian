...
class lhc {
	function drawtimes(){
      /*  $url="http://1680660.com/smallSix/findSmallSixInfo.do";//获取当月开奖时间
        ...
        */
		$res=M('lhctime')->where(['drawTime'=>['gt',time()]])->order('id asc')->find();
		//var_dump($res);
		 $lastFullExpect =$res['preDrawIssue'];//上期
        $currFullExpect = $res['qihao'];//下期
        $openRemainTime = $res['drawTime'];//下期时间
		$name = 'lhc';
		...
		$return = [
			...
			'remainTime'      => ($openRemainTime)-time()-$cjnowtime,
			'openRemainTime'  => $cjnowtime,
		];
		return $return;
	}

 function curl($curlurl){
        $ch = curl_init();
        $referurl = "https://www.cdyxxd.com/home/lottery_code/index/lottery_id/32.html";
