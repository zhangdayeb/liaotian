...
use app\common\model\mysql\YwBeautiful as models;
use app\common\model\mysql\CommonAdmin as commonModels;
use app\common\model\mysql\CommonLog;
use app\common\validate\YwBeautiful as ValidateModel;
use think\exception\ValidateException;

class YwBeautiful extends BaseController
{
    ...
    public function edit(){
        $fildes = "token,id,city_id,image,remark,name,status,order_num";
        $receiveData = getData($this->request->post(),$fildes);
        //校验token
        ...
        if(!empty($receiveData['id'])){
            ...
            //写入日志
            CommonLog::insertLog($admin['id'],'/yw-beaufiful/edit','',$data,2);
        }else{
            $data['create_time'] = date('Y-m-d H:i:s');
            models::insert($data);
            //写入日志
            CommonLog::insertLog($admin['id'],'/yw-beaufiful/edit','',$data,1);
        }
        jsonSuccess();
    }
    ...
    public function delete(){
        ...
        //写入日志
        CommonLog::insertLog($admin['id'],'/yw-beaufiful/delete',$load,'',3);
        jsonSuccess();
    }
